...
function installClaude(): void {
  ...
  if (existsSync(target) && !readFileSync(target, "utf8").includes("Generated by Drax Corp")) {
...
function installLauncher(): void {
  const runtimeRoot = path.join(home, ".local", "share", "drax-corp");
...
function install(target: string): void {
  ...
  installLauncher();
  console.log(`Installed Drax Corp ${VERSION} for ${target}.`);
...
function doctor(): void {
  const checks = [
    ...
    ["Persistent runtime", path.join(home, ".local", "share", "drax-corp", "dist", "cli.js")],
  ] as const;
  let ok = true;
