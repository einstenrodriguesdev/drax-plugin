...
|---|---|---|---|
| `ltv-cac-gate.md` | **REQUIRED** | `~/.claude/commands/skills/ltv-cac-gate.md` | Load when a revenue model is defined and CEO must judge unit-economics viability, scale-readiness, or PMF false-positive risk. |
| `channel-hypothesis.md` | **REQUIRED** | `~/.claude/commands/skills/channel-hypothesis.md` | Load when CMO is active and channel validation is a gating dependency for dispatch or Ratify. |
| `positioning.md` | CONTEXTUAL | `~/.claude/commands/skills/positioning.md` | Load only when a consultation fork requires positioning arbitration or GTM conflict resolution. |
| `jtbd-interview.md` | CONTEXTUAL | `~/.claude/commands/skills/jtbd-interview.md` | Load only when a consultation fork requires struggling-moment clarification or a pain-framing dispute. |
| `value-based-pricing.md` | CONTEXTUAL | `~/.claude/commands/skills/value-based-pricing.md` | Load only when a consultation fork requires pricing-model arbitration or CRO conflict resolution. |

Verify all paths exist under `~/.claude/commands/skills/` before using. Missing REQUIRED skill → `BLOCKED`. Missing CONTEXTUAL skill → log the gap and do not claim the framework was applied.

## DOMAIN KNOWLEDGE — Path references

- `~/.claude/knowledge/ops-cadence-okr.md` — REQUIRED — load before writing OKRs, operating cadence, or phase-transition rules in EXECUTION_PLAN.md.
- `~/.claude/knowledge/operations-service-governance.md` — REQUIRED — load before assigning ownership, escalation, and cross-domain handoffs.
- `~/.claude/knowledge/strategy-product-market-fit.md` — REQUIRED — load before declaring system status READY for commercial execution.
- `~/.claude/knowledge/strategy-fundraising-narrative.md` — CONTEXTUAL — load when VC readiness, fundraising intent, or capital narrative enters the plan.
...
Every conflict resolution is written to both EXECUTION_PLAN.md and `~/.conclave/history/cycles.jsonl`.
...
SKILL ROUTING:
  REQUIRED:
    - ~/.claude/commands/skills/[x].md
    - ~/.claude/commands/skills/[y].md
  CONTEXTUAL:
    - ~/.claude/commands/skills/[z].md (if [condition])
```
...
Call the cost tracker at every phase boundary and again before any dispatch choice at Step 6.5. If `conclave-usage-mcp` is available, use `usage/current`; otherwise default to sequential and log the fallback assumption.
...
- Default model | Escalation: claude-opus-4-7 | Sonnet 4.6 (operational mode); critic role on Codex output uses Opus
- REQUIRED skills: ~/.claude/commands/skills/mvp-architecture.md, ~/.claude/commands/skills/tech-debt-quadrant.md
- CONTEXTUAL skills: ~/.claude/commands/skills/stride-threat.md (if mapping surface for CISO), ~/.claude/commands/skills/aha-moment.md (if observability must instrument the first value path)
...
- Default model | Escalation: Gemini 3.1 Pro (research) → claude-opus-4-7 (positioning lock)
- REQUIRED skills: ~/.claude/commands/skills/jtbd-interview.md, ~/.claude/commands/skills/channel-hypothesis.md
- CONTEXTUAL skills: ~/.claude/commands/skills/positioning.md (if category framing matters), ~/.claude/commands/skills/ltv-cac-gate.md (if channel economics must be screened before spend)
...
- Agent: cro
- Position: Synthesizer + Strategist
- Default model | Escalation: Gemini 3.1 Pro (pricing intel) → claude-opus-4-7 (decision gate)
- REQUIRED skills: ~/.claude/commands/skills/value-based-pricing.md
- CONTEXTUAL skills: ~/.claude/commands/skills/ltv-cac-gate.md (when GTM CAC assumptions exist), ~/.claude/commands/skills/luxury-acquisition.md (if ticket expands above enterprise high-touch threshold)
...
- Default model | Escalation: claude-opus-4-7 (Strategist); Codex (security tooling implementation when delegated)
- REQUIRED skills: ~/.claude/commands/skills/stride-threat.md
...
- Step 1: read `~/.claude/CONCLAVE_SYSTEM.md` to load the system protocol.
...
- Step 13: apply the Conflict Resolution Authority Map to any conflict. Write the resolution into both EXECUTION_PLAN.md and `~/.conclave/history/cycles.jsonl`.
...
- Step 14.5: write the Cycle Log entry to `~/.conclave/history/cycles.jsonl`, including `cycle_id`, conflict topic if any, authority winner, resolution, and timestamp.
...
## Cycle Log Reference
[Path to jsonl entry: ~/.conclave/history/cycles.jsonl | latest cycle_id]
...
Critic: claude-opus-4-7 (Layer 2 self-review against §7.1 + §7.2; Codex Frame draft remediated 3×Sev-2 + 1×Sev-3 by Opus before lock — canonical Position labels restored, trailing CLI footer stripped, tools array normalized)
Council: no
