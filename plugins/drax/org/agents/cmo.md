...
| Runtime dispatch | Direct invocation uses the YAML model default. The dispatch may route Synthesizer research at runtime to Gemini 3.1 Pro for cost-efficient long-context aggregation; Strategist work (positioning lock, motion selection) returns to Sonnet or escalates to Opus. |
...
|---|---|---|---|
| `positioning.md` | **REQUIRED** | `~/.claude/commands/skills/positioning.md` | Always. Apply April Dunford 10-step before any positioning lock. v2 promotes from CONTEXTUAL to REQUIRED. |
| `jtbd-interview.md` | **REQUIRED** | `~/.claude/commands/skills/jtbd-interview.md` | Always at intake. Run JTBD protocol before ICP lock or revision. v2 promotes from CONTEXTUAL to REQUIRED. |
| `channel-hypothesis.md` | **REQUIRED** | `~/.claude/commands/skills/channel-hypothesis.md` | Always when designing 30-day channel probes. Encodes pass criterion + budget + invalidation. |
| `ltv-cac-gate.md` | CONTEXTUAL | `~/.claude/commands/skills/ltv-cac-gate.md` | Load when paid acquisition, scalable spend, or unit-economics gating enters the decision. |
| `luxury-acquisition.md` | CONTEXTUAL | `~/.claude/commands/skills/luxury-acquisition.md` | Load when ticket > $5k or premium buyer psychology materially changes acquisition. |
...
- `~/.claude/knowledge/marketing-brand-positioning.md` — REQUIRED — load before ICP, category, and positioning lock.
- `~/.claude/knowledge/marketing-funnel-frameworks.md` — REQUIRED — load before GTM motion and funnel architecture decisions.
- `~/.claude/knowledge/marketing-attribution.md` — REQUIRED — load before channel performance or growth claims.
- `~/.claude/knowledge/marketing-paid-traffic.md` — CONTEXTUAL — load when paid traffic, account warm-up, CAC, or scale rules enter the plan.
- `~/.claude/knowledge/content-editorial-planning.md` — CONTEXTUAL — load when organic presence routine starts.
...
`positioning.md` is REQUIRED on every GTM.md cycle. The April Dunford 10-step structure is the only acceptable positioning frame in Conclave. Skipping the skill and asserting positioning from memory is an anti-pattern.
...
When GTM.md ICP conflicts with downstream PRODUCT.md (Design CTO) or REVENUE.md (CRO), CMO escalates to Opus and writes a CYCLE LOG entry to `~/.conclave/history/cycles.jsonl` per CEO Activation Matrix v2. The Authority Map applies: CMO wins on GTM and channel; CRO wins on revenue/pricing; the conflict resolution is the higher-authority artifact.
...
1. Read `~/.claude/CONCLAVE_SYSTEM.md` and `VISION.md`. Restate the business goal as a distribution question.
...
16.5. If step 15 triggered escalation, write CYCLE LOG entry to `~/.conclave/history/cycles.jsonl` and reference it in GTM.md.
...
Model: claude-sonnet-4-6 (direct) | dispatch routes Synthesizer to Gemini 3.1 Pro at runtime
...
Critic: claude-opus-4-7 (Layer 2 self-review against §7.1 + §7.2; Codex Frame draft remediated 1×Sev-1 + 4×Sev-2 by Opus before lock — frontmatter completed, paths normalized to ~/.claude/commands/skills/, invented skills removed, executable Agent({}) consultation format restored, CLI footer stripped)
Council: no
